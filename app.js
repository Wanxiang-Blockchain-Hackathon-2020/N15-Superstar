 ...
 */
'use strict';
var log4js = require('log4js'); //로그 출력
...
//Login Request를 날리면 가장 먼저 실행되는 부분
app.use(bearerToken());

app.use(function(req, res, next) {
	logger.debug(' ------>>>>>> new request for %s',req.originalUrl);
    
	if (req.originalUrl.indexOf('/users') >= 0) { //indexOf는 특정 문자열의 위치를 반환하는데 없으면 -1을 반환한다. 
		return next();
	}
...
app.post('/users', async function(req, res) {
	...
	let response = await helper.getRegisteredUser(username, orgName, true); // isJson == true
...
app.post('/channels/:channelName/chaincodes/:chaincodeName', async function(req, res) {
    ...
    //app단에서 json이 String으로 넘어와 처리하였다. 
    try{
        //String 형태로 넘어온 데이터를 Json으로 변환한다. 
        var temp = JSON.parse(args);    
        args = temp;
    } catch(e){
        //이미 args가 Json 형태면 catch 발생 
    }
    
    try{
        //String 형태로 넘어온 데이터를 Json으로 변환한다. 
        var temp = JSON.parse(peers);    
        peers = temp;
    } catch(e){
    ...
    if(fcn=="query"){
        let message = await query.queryChaincode("peer0.org1.example.com", channelName, chaincodeName, args, fcn, req.username, req.orgname);
        logger.debug(message);
	   res.send(message);
    }else if(fcn=="queryRange"){
        let message = await query.queryChaincode("peer0.org1.example.com", channelName, chaincodeName, args, fcn, req.username, req.orgname);
        logger.debug(message);
	   res.send(message);
    }else{
